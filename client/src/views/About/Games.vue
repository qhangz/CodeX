...
onMounted(() => {
    ...
    const restartButton = createElementStyle(
        "button",
        `width:120px;height:120px;position:absolute;border-radius:50%;color:white;background-color:#CC231E;border:none;font-weight:700;font-size:1.2em;display:none;cursor:pointer;z-index:999;`,
        "重新开始"
    );
    ...
    function createElementStyle(element, cssStyles, inner) {
        const newElement = document.createElement(element);
        if (cssStyles) {
            // newElement.style.cssText = "position: relative; z-index: 1; " + cssStyles;
            newElement.style.cssText = cssStyles;
        }
        if (inner) {
